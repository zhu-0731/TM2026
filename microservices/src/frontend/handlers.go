...
func (fe *frontendServer) placeOrderHandler(w http.ResponseWriter, r *http.Request) {
	...
	// 从 response metadata 读取优惠券信息（手动添加的 proto 字段在序列化中丢失）
	var couponDiscount *pb.Money
	log.Infof("DEBUG header: coupon-code=%v, units=%v, nanos=%v", header.Get("x-coupon-code"), header.Get("x-coupon-discount-units"), header.Get("x-coupon-discount-nanos"))
	if h := header; len(h.Get("x-coupon-code")) > 0 {
		if unitsStr := h.Get("x-coupon-discount-units"); len(unitsStr) > 0 {
			units, _ := strconv.ParseInt(unitsStr[0], 10, 64)
			nanosStr := h.Get("x-coupon-discount-nanos")
			nanos, _ := strconv.ParseInt(nanosStr[0], 10, 32)
			couponDiscount = &pb.Money{
				CurrencyCode: currentCurrency(r),
				Units:        units,
				Nanos:        int32(nanos),
			}
			log.Infof("DEBUG parsed discount: units=%d, nanos=%d", units, nanos)
		} else {
			log.Infof("DEBUG: x-coupon-discount-units not found in header")
		}
	} else {
		log.Infof("DEBUG: x-coupon-code not found in header")
	}
	...
	if couponDiscount != nil {
		totalPaid = money.Must(money.Sum(totalPaid, money.Negate(*couponDiscount)))
	}
...
func renderMoneyPtr(money *pb.Money) string {
	if money == nil {
		return "$0.00"
	}
